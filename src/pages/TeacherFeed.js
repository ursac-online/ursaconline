...
export default function TeacherFeed() {
  ...
  const date = new Date();
  const today = format(date, "yyyy-MM-dd 23:59");
  ...
  const getClassrooms = () => {
    ...
    axios
      .post(
        "https://ursacapi.000webhostapp.com/api/getClassroomID.php",
  ...
  const streamFeed = () => {
    axios
      .post(
        "https://ursacapi.000webhostapp.com/api/getPost.php",
        JSON.stringify(id)
      )
      .then((response) => {
        if (response.data == 0) {
  ...
  const handleChange = (event) => {
    ...
    setPostAsActivity((postAsActivity) => ({
      ...postAsActivity,
      [name]: value,
    }));
  };
  ...
  const [postAsActivity, setPostAsActivity] = useState({
    points: "",
    dueDate: today,
  });
  ...
  const uploadFile = async (filePreview) => {
    ...
    formdata.append("isAnActivity", isTrue);
    formdata.append("points", postAsActivity.points);
    formdata.append("dueDate", postAsActivity.dueDate);
        ...
        >
                ...
                <form onSubmit={handlePost}>
                  ...
                  <label htmlFor="attach-file">
                    <IconButton component="span">
                      <Tooltip title="Attach a file" placement="right-start">
                        <AttachFileRounded />
                      </Tooltip>
                    </IconButton>
                  </label>
          ...
          {noPost ? (
            <Paper variant="outlined">
              <MenuItem disabled>You have no announcement yet.</MenuItem>
            </Paper>
          ) : (
